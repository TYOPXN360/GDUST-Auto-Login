...
### 1. 准备脚本
在release板块下载zip,解压到任意文件夹,右键以Powershell运行即可


### 2. 运行方式

#### 方式1：交互式运行（默认）
在终端中直接运行脚本或在PowerShell中执行：
```powershell
.\authVX.X.ps1
```
...
2. **账号密码配置**：  
   脚本中默认账号密码为空，使用时会提示输入自己的校园网账号（通常为校园卡号码）,密码为身份证后六位。

3. **校区适配说明**：  
   脚本仅针对南城校区校园网认证服务器（`172.18.100.100` 等）优化，其他校区可能需要调整服务器地址。
